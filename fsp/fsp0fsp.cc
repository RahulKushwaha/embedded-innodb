...
page_no_t FSP::fseg_alloc_free_page_low(
  ...
  if (xdes_get_state(hint_descr, mtr) == XDES_FSEG && mtr->read_uint64(hint_descr + XDES_ID) == seg_id && xdes_get_bit(hint_descr, XDES_FREE_BIT, hint % FSP_EXTENT_SIZE, mtr)) {
  ...
  } else if (xdes_get_state(hint_descr, mtr) == XDES_FSEG && mtr->read_uint64(hint_descr + XDES_ID) == seg_id && !xdes_is_full(hint_descr, mtr)) {
