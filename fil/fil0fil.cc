...
mutex. */
static void
fil_node_prepare_for_io(fil_node_t *node,     /** in: file node */
...
fil_write(bool sync,      /** in: true if synchronous aio is desired */
          ...
          void *message)     /** in: message for aio handler if non-sync
                             aio used, else ignored */
{
  return fil_io(OS_FILE_WRITE, sync, space_id, block_offset, byte_offset, len,
                buf, message);
}
    ...
    lsn_t lsn)          /** in: lsn to write */
{
  ...
  fil_read(true, 0, SYS_TABLESPACE, sum_of_sizes, 0, UNIV_PAGE_SIZE, buf,
           nullptr);

  mach_write_to_8(buf + FIL_PAGE_FILE_FLUSH_LSN, lsn);

  fil_write(true, 0, SYS_TABLESPACE, sum_of_sizes, 0, UNIV_PAGE_SIZE, buf,
            nullptr);
...
static char *fil_make_ibd_name(const char *name, /** in: table name or a dir
                                                 path of a TEMPORARY table */
